<?php
    ...
    $s_phone = $json->phone;
    $phone = delete_all_between('<', '>',$s_phone);
    $s_name = $json->name;
    ...
    $qr = "SELECT * FROM `users` WHERE `user_phone` = :phone AND `user_name` = :name";
    $pr = array(
        "phone" => $phone,
        "name" => $name
    );
    ...
    if($count == 0) {
        if($password === $confirm) {
            ...
            $qr = "INSERT INTO users (user_phone, user_name, user_salt, user_password) VALUES (:phone, :name, :salt, :password)";
            $pr = array(
                "phone" => $phone,
                "name" => $name,
                "salt" => $salt,
            ...
            db::add($qr, $pr);
            $qr = "SELECT id_user FROM `users` WHERE `user_phone` = :phone";
            $pr = array(
                "phone" => $phone
            );
            ...
            db::add($qr, $pr);
            $qr = "SELECT * FROM `users` WHERE user_phone = :phone";
            $pr = array(
                "phone" => $phone
            );
